...
const LeafletMap: React.FC<LeafletMapProps> = ({ supervisions }) => {
  ...
  const [pathPolylines, setPathPolylines] = useState<Polyline<any>[]>(
    [] as Polyline<any>[]
  );
  ...
  useEffect(() => {
    ...
    if (map) {
      ...
      pathPolylines.forEach((polyline: Polyline<any>) => {
        objectsToFit.push(polyline);
      });
  ...
  }, [observationMarkerPairs, map, supervisions]);
  ...
  useEffect(() => {
    ...
    setPathPolylines(newPathPolylines);
  }, [supervisions, map]);
